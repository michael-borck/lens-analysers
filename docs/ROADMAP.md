...
- auto-analyser: add cascade rules beyond image→diagram; validate preset member signal-richness — **DONE 2026-09-16** (0.8.0): multi-rule cascades + document→provenance/conversation/reflection heuristics (config-gated via `cascades.enabled`); bundle-analyser 0.6.0 promotes cascade blocks to first-class labelled results (`via`), so **explicit-only members now flow through bundle-driven runs**. Desktop: heuristic second passes user-gated on the Setup screen.
